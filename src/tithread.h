	  ...
	  for(uint32_t i = seedwin; i < hdr->target_len[refIndex] - seedwin; ++i) {
		...
		if ((lcov * (c.sdthres / 2) < rcov) && (rcov > avgcov + c.sdthres * sdcov)) {
		  ...
		  if (controlrcov > 0) {
		    ...
		    if (obsratio / expratio > (c.sdthres / 2)) {
		      //std::cerr << "Left: " << i << ',' << left[i] << std::endl;
		      bpvec.push_back(Breakpoint(true, i, left[i], obsratio / expratio));
		    }
		  }
		}
	      }
		...
		if ((rcov * (c.sdthres / 2) < lcov) && (lcov > avgcov + c.sdthres * sdcov)) {
		  ...
		  if (controllcov > 0) {
		    ...
		    if (obsratio / expratio > (c.sdthres / 2)) {
		      //std::cerr << "Right: " << i << ',' << right[i] << std::endl;
		      bpvec.push_back(Breakpoint(false, i, right[i], obsratio / expratio));
		    }
		  }
		}
	      }
